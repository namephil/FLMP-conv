...
import numpy as np
from modules import Encoder, LayerNorm, SelfAttention
from Layer import SequenceAttLayer
class FMLPRecModel(nn.Module):
    def __init__(self, args):
        ...
        self.conv_v = nn.Conv2d(1, 8, (args.max_seq_length, 1))
        self.fc = nn.Linear(512, 100)
        # self.self_attention = SequenceAttLayer()
    ...
    def attention_position_embedding(self,sequence, extended_attention_mask):
        item_embeddings = self.item_embeddings(sequence)
        output = self.self_attention(item_embeddings, extended_attention_mask)
        output = item_embeddings + output
        return output
